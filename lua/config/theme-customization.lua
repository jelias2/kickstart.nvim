...
local function updateHl(hlgroup, changes)
  vim.cmd.highlight(hlgroup .. ' ' .. changes)
end

---@param hlgroup string
---@param changes vim.api.keyset.highlight
local function setHl(hlgroup, changes)
  vim.api.nvim_set_hl(0, hlgroup, changes)
end
...
function M.themeModifications()
  ...
  local function revertedTodoComments()
    ...
    for type, altType in pairs(types) do
      local fg = u.getHlValue('@comment.' .. type, 'fg') or u.getHlValue('Diagnostic' .. altType, 'fg')
